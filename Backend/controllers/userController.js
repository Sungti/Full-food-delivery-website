...
exports.postCart = (req, res, next) => {
  ...
  if (!itemId) {
    ...
    error.statusCode = 400; // Correct status code for bad request
    throw error;
  }
...
exports.getCart = async (req, res, next) => {
  try {
    ...
    const user = await User.findOne({ account: account._id }).populate("cart.items.itemId");
...
exports.postOrder = async (req, res, next) => {
  try {
    ...
    if (!user.address) {
      const error = new Error('Order validation failed: user.address is required.');
      error.statusCode = 422;
      throw error;
    }

    // Group items by seller
    const sellers = user.cart.items.reduce((acc, item) => {
      const sellerId = item.itemId.creator.toString();
      if (!acc[sellerId]) {
        acc[sellerId] = [];
      }
      acc[sellerId].push({
        quantity: item.quantity,
        item: { ...item.itemId._doc }, // Use ._doc to access document fields
      });
      return acc;
    }, {});

    // Create orders for each seller
    const orderPromises = Object.entries(sellers).map(async ([sellerId, items]) => {
      ...
      // Emit orders event to seller's socket clients
      for (const clientId of Object.keys(app.clients)) {
        if (clientId.toString() === seller._id.toString()) {
          io.getIO().sockets.connected[app.clients[clientId].socket].emit("orders", { action: "create", order: order });
        }
      }
    ...
    // Wait for all orders to be saved and emitted
    const orders = await Promise.all(orderPromises);

    // Clear user's cart after orders are successfully created
